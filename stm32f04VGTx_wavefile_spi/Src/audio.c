...
uint32_t timeAudCap[256];
uint32_t timeAudCapCnt=0;
...
void  AudioPlay_HalfTransfer_CallBack(void) {
  ...
  packet_audio_play(1);
  timeAudCap[timeAudCapCnt] = HAL_GetTick();
}

void  AudioPlay_TransferComplete_CallBack(void) {
    buffer_offset = BUFFER_OFFSET_FULL;
    if(!Is_cs43l22_Stop) {
    	timeAudCap[timeAudCapCnt] = HAL_GetTick() - timeAudCap[timeAudCapCnt];
    	timeAudCapCnt = (timeAudCapCnt + 1) % 256;
    	packet_audio_play(2);
